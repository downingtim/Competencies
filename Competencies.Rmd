...
my_data_long <- my_data_long %>% mutate(Competency = case_when(
    grepl("\\.A3\\.", Original_Column) ~ "A",
    grepl("\\.B3\\.", Original_Column) ~ "B",
    grepl("\\.C3\\.", Original_Column) ~ "C",
    grepl("\\.D3\\.", Original_Column) ~ "D",
    grepl("\\.E3\\.", Original_Column) ~ "E",
    grepl("\\.F3\\.", Original_Column) ~ "F",
    grepl("\\.G3\\.", Original_Column) ~ "G",
    grepl("\\.H3\\.", Original_Column) ~ "H",
    grepl("\\.I3\\.", Original_Column) ~ "I",
    grepl("\\.J3\\.", Original_Column) ~ "J",
    grepl("\\.K3\\.", Original_Column) ~ "K",
    grepl("\\.L3\\.", Original_Column) ~ "L",
    grepl("\\.M3\\.", Original_Column) ~ "M",
    grepl("New..Identify.and.support.users..needs.", Original_Column) ~ "N",
    grepl("New..Project.management.", Original_Column) ~ "O",
    grepl("New..People.management..focusing.on.staff..",Original_Column)~"P",
    grepl("New..Collaborator.engagement.", Original_Column) ~ "Q",
    grepl("New..Training.", Original_Column) ~ "R",
    grepl("New..Leadership.", Original_Column) ~ "S" ))
...
# for plotting
competency_colors <- c(
  "A" = "#F28E82",  # Light pink/red
  "B" = "#F4A261",  # Orange
  "C" = "#E9C46A",  # Yellow
  "D" = "#A8D08D",  # Green
  "E" = "#A5A58D",  # Olive
  "F" = "#8EC07C",  # Light green
  "G" = "#89C4C8",  # Light cyan
  "H" = "#52B788",  # Deep green
  "I" = "#00AFFF",  # Light blue
  "J" = "#8B9DC3",  # Light purple
  "K" = "#C39BD3",  # Lavender
  "L" = "#B587FF",  # Light purple
  "M" = "pink",  # Light pink/purple
  "N" = "black",  # Light blue/purple
  "O" = "grey70",  # Light purple
  "P" = "grey40",  # Pink
  "Q" = "yellow",  # Pink/purple
  "R" = "yellowgreen",  # Rose pink
  "S" = "#E9C35B")   # Deep pink
...
pdf("Fig_5_Competency_means.pdf", width = 4, height = 5)
mean_scores_ci %>%  filter(Level == 1) %>%
  ggplot(aes(x = Competency, y = Mean_Score, group = Competency,
             color = Competency)) + 
  geom_point(size = 1.8, alpha = .8, aes(shape = "I")) + 
  geom_point(data = m2, aes(x = Competency, y = Mean_Score,
        color = Competency, shape = "II"), size = 1.8, alpha = .8) + 
  geom_point(data = m3, aes(x = Competency, y = Mean_Score,
        color = Competency, shape = "III"), size = 1.8, alpha = .8) + 
  geom_point(data = m4, aes(x = Competency, y = Mean_Score,
      color = Competency, shape = "IV managerial"), size = 1.8, alpha = .8) + 
  labs(x = "Stage", y = "Mean Score") +  ylim(0, 3) + theme_minimal() +  
  theme(legend.position = "bottom",  legend.box = "vertical", 
        strip.text = element_text(size = 12, face = "bold")) +
  scale_color_manual(values=competency_colors, guide=guide_legend(order=1))+
  scale_shape_manual( name = "Career",
    values=c("I" = 1, "II"=6, "III"=7, "IV managerial"=8),
