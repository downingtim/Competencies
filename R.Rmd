...
my_data_long <- my_data_long %>% mutate(Competency = case_when(
    ...
    grepl("New..Identify.and.support.users..needs.", Original_Column) ~ "N3",
    grepl("New..Project.management.", Original_Column) ~ "O3",
    grepl("New..People.management..focusing.on.staff..", Original_Column) ~ "P3",
    grepl("New..Collaborator.engagement.", Original_Column) ~ "Q3",
    grepl("New..Training.", Original_Column) ~ "R3",
    grepl("New..Leadership.", Original_Column) ~ "S3" ))
...
View(my_data_long2)
write.csv(my_data_long2, "cleaned_long_data.csv", row.names = FALSE)
...
View(set1)
write.csv(set1,  "correlations.csv")
...
# Create the plot with confidence intervals and global dashed mean line
pdf("Competency_change_with_CI.pdf", width=6, height=6)
ggplot(mean_scores_ci, aes(x=Level, y=Mean_Score, group=Competency,color=Competency)) +
  # Add the confidence interval as a shaded area
  geom_ribbon(aes(ymin = Lower_CI, ymax = Upper_CI), fill="grey", alpha=0.2) +
  geom_line(linewidth=1.3) +  # Draw lines for each competency
  geom_point(size=1.2, alpha=0.8) +  # Add points for better visibility
  # Add global mean line as a dashed black line, disable inherited aesthetics
  geom_line(data=mean_level2, aes(x=Level, y=Mean_Score), color="black",
            linewidth=0.8, linetype="dashed", inherit.aes = F) +
  facet_wrap(~ Competency, labeller = label_wrap_gen(width = 25)) +  
  labs(x = "Stage", y = "Mean Score") + ylim(1, 3) +
...
#    O3 - somewhat correlated with later stages - "New..Project.management."
#    P3 - very correlated with later stages - "New..People.management..focusing.on.staff.
