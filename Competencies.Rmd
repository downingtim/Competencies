...
### pairwise correlations # not useful
...
correlation_test <- function(data) {
  ...
  for (i in 1:(n-1)) {
    for (j in (i+1):n) {
      res <- cor.test(unlist(data[,i]),unlist(data[,j]), method="pearson",
...
# get top ones
subset(cor_melt, value>0.75)
# get lower ones
subset(cor_melt, value<.3)
...
text(pca_result$x[, 1], pca_result$x[, 2], labels = rownames(pca_result$x), 
     pos = c(3,1,1,1,1,1,3,1,3,1,2,1,1,4,1,1,1,2,3), cex = 1.3,
     col = competency_colors[rownames(pca_result$x)]) 
