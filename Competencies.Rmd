...
mean_scores_ci <- mean_scores_ci %>%
  mutate(Competency_Group = case_when(
    Competency %in% c("A3", "B3", "C3") ~ "Group 1: A-C",
    Competency %in% c("D3", "E3", "F3", "G3", "H3", "I3", "J3", "K3", "L3", "M3") ~ "Group 2: D-M",
    Competency %in% c("N1", "O1", "P1", "Q1", "R1", "S1") ~ "Group 3: N-S" ))

m2 <- mean_scores_ci %>% filter(Level == 2)
m3 <- mean_scores_ci %>% filter(Level == 3)
m4 <- mean_scores_ci %>% filter(Level == 4)
pdf("Fig_5_Competency_means.pdf", width = 6, height = 5)
...
pdf("Fig_5_Competency_means_transposed.pdf", width =5, height =6)
mean_scores_ci %>% 
  filter(Level == 1) %>%
  ggplot(aes(y = Competency, x = Mean_Score, group = Competency)) + 
  geom_point(size = 5, alpha = 0.5, aes(color = "I", shape = "I", fill = "I"), show.legend = TRUE) +
  geom_point(data = m2, aes(y = Competency, x = Mean_Score, color = "II", shape = "II", fill = "II"), 
             size = 5, alpha = 0.5, show.legend = TRUE) +
  geom_point(data = m3, aes(y = Competency, x = Mean_Score, color = "III", shape = "III", fill = "III"), 
             size = 5, alpha = 0.5, show.legend = TRUE) +
  geom_point(data = m4, alpha = 0.7, aes(y = Competency, x = Mean_Score, color = "M managerial", shape = "M managerial", fill = "M managerial"), 
             size = 5, stroke = 1.5, show.legend = TRUE) +  
  geom_text(data = m4, aes(y = Competency, x = Mean_Score, label = "M"), 
            color = "black", size = 4, fontface = "bold", show.legend = FALSE) +
  labs(y = "Competency", x = "Mean Score") +  
  xlim(0, 3) +  
  theme_minimal() +  
  theme(
  ...
  scale_color_manual(
    ...
    values = c("I" = "lightblue4", "II" = "blue", "III" = "navy", "M managerial" = "orange")
  ) +
  scale_fill_manual(
    name = "Career", 
    guide = guide_legend(order = 1),
  ...
  scale_y_discrete(
    limits = function(x) rev(x)  # Dynamically reverse order for each group
  )
